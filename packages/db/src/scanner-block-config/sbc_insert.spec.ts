...
describe('Test insert in scanner_block_config', () => {
    ...
    it('test insert is working', async () => {
        const previousCount = await knex(scannerBlockConfigTableName).count(
            'scannerId'
        );
        ...
        const currentCount = await knex(scannerBlockConfigTableName).count(
            'scannerId'
        );
        expect(BigInt(currentCount[0]['count'])).to.equal(
            BigInt(previousCount[0]['count']) + BigInt(1)
        );
    });
});
