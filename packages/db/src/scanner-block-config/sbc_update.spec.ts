...
describe('Test get in scanner_block_config', () => { 
    ...
    it('test update', async () => {
        await updateCurrentBlockHeight(knex, {
                ...
                currentBlockHeight: Number(scanner_block_config.currentBlockHeight) + 10
        });
        const scannerBlockConfig = await getScannerConfig(knex, scanner_block_config.scannerId);
        expect(BigInt(scannerBlockConfig?.currentBlockHeight as string)).to.eq(BigInt( Number(scanner_block_config.currentBlockHeight) + 10));
    })
})