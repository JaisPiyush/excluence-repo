...
export class UniqueEventBroadcaster implements EventBroadcasterInterface {
  constructor(
    private readonly uniqueCheckerProvider: UniqueCheckerProvider,
    private readonly broadcaster: EventBroadcasterInterface,
  ...
  broadcastEvents = async (blockHeight: number, events: FlowFetchedEvent[]) => {
    ...
    for (const transactionId in groups) {
      ...
      try {
        ...
        if (!consumed) {
          await this.broadcaster.broadcastEvents(
            blockHeight,
          ...
          this.logProvider().debug(
            `Skipping transaction already sent: ${transactionId}`
          );
        }
