...
export class SqliteUniqueChecker
  implements UniqueCheckerInterface<SqliteLockData>
{
  private db: Knex | undefined = undefined;

  constructor(
    private readonly filename: string,
    private readonly groupId: string | undefined = undefined,
    private readonly lockTtlMs = 10000
  ) {
  ...
  private initializeDb = async (db: Knex) => {
    ...
    if (!metaTableCheck) {
      ...
      const versionRecord = await db('meta').where('key', 'version').first();

      await this.migrateDb(
        db,
        versionRecord?.value ? Number(versionRecord.value) : 0
      );
    }
  };
  ...
  acquireLock = async (key: string): Promise<SqliteLockData | undefined> => {
    ...
    await db.transaction(async (trx) => {
        ...
        .where((builder) => {
          builder
            ...
            .orWhere(
              'lock_timestamp',
              '<',
              new Date().getTime() - this.lockTtlMs
            );
        })
        .update({
          lock_id: lockId,
      ...
      const lock = await trx('records').where('key', uniqueKey).first();
  ...
  checkConsumed = async (lockData: SqliteLockData): Promise<boolean> => {
    ...
    const record = await db('records').where('key', lockData.key).first();
