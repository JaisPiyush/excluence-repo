...
export class CloudwatchMetricService implements MetricServiceInterface {
    ...
    constructor(
        ...
        private readonly metricEnv: string,
    ...
    putMetric = (
        ...
        if (statistic) {
            let min = Math.min(
                this.queue[key].metric?.statisticValues?.min ?? value,
                value
            );
            let max = Math.max(
                this.queue[key].metric?.statisticValues?.max ?? value,
                value
            );
            let sum =
                (this.queue[key].metric?.statisticValues?.sum ?? 0) + value;
            let sampleCount =
                (this.queue[key].metric?.statisticValues?.sampleCount ?? 0) + 1;
            ...
            const newValue = overwrite
                ? value
                : (this.queue[key].metric?.value ?? 0) + value;
            ...
            if (queues.hasOwnProperty(queueKey)) {
                ...
                if (queue.metric) {
                    ...
                    while (true) {
                        try {
                            logger.debug(
                                `Sending metric for ${namespace}/${
                                    metric.metricName
                                } (${
                                    metric.value !== undefined
                                        ? metric.value
                                        : metric.statisticValues
                                        ? JSON.stringify(metric.statisticValues)
                                        : '-'
                                })`
                            );
