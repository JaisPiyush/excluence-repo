      ...
      if (this.latestBlockHeight) {
        ...
        if (startHeight <= endHeight) {
          ...
          while (true) {
            ...
            try {
              logger.debug(
                `Fetching events: ${this.eventType} ${startHeight}-${endHeight}`
              );
              ...
              const groupedEvents: { [key: string]: FlowEvent[] } = _.groupBy(
                events,
                (e) => String(e.blockHeight)
              );
