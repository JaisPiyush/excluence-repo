...
import {
    flowServiceProvider,
    FlowServiceProvider
} from '../providers/flow-service-provider';
import {
    eventBusProvider,
    EventBusProvider
...
import {
    ...
    nullMetricServiceProvider
} from '../providers/metric-service-provider';
...
export class FlowScanner {
    ...
    constructor(providers: ProviderOptions) {
        this.providers = {
            ...
            flowServiceProvider: flowServiceProvider(
                flowClientProvider(providers.configProvider),
                flowRateLimiterProvider(providers.configProvider)
            ),
            eventBusProvider: eventBusProvider,
    ...
    stop = async () => {
        ...
        const eventBroadcaster =
            await this.providers.eventBroadcasterProvider();
    ...
    private process = async () => {
        ...
        try {
            ...
            const settingsService =
                await this.providers.settingsServiceProvider();
            ...
            while (!missing) {
                ...
                if (!missing) {
                    ...
                    if (allEvents.length) {
                        const eventBroadcaster =
                            await this.providers.eventBroadcasterProvider();
                        await eventBroadcaster.broadcastEvents(
                            checkBlockHeight,
                            allEvents
                        );
                    }
                    ...
                    try {
                        ...
                        metricService.putMetric(
                            ...
                            new Date().getTime() - startProcessingTime,
                            true,
                            false,
                            false
                        );
                    ...
                    eventBus.emit<EventPayloads.ProcessedBlockHeightUpdated>(
                        ...
                        { blockHeight: this.processedBlockHeight }
                    );
                }
            }
        } catch (err) {
