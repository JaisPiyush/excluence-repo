...
describe('Test flow scanner', () => {
  it('Test that the scanner creates the configured event scanners', async () => {
    ...
    const flowScanner = new FlowScanner(['test1', 'test2', 'test3'], {
  ...
  it('Test broadcasting of events', async () => {
    ...
    for (const broadcast of broadcasts) {
      ...
      expect(broadcast.events).length(
        _.sumBy(testBlock.transactions, (t) => t.events.length)
      );

      for (const event of broadcast.events) {
        const testEvent =
          testBlock.transactions[event.transactionIndex].events[
            event.eventIndex
          ];
