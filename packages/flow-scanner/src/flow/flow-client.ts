...
import { FlowTransactionObject, FlowTransactionStatus } from "./models/flow-transaction";
...
export class FlowClient implements FlowClientInterface {
    ...
    getTransactionObject = async (transactionId: string): Promise<FlowTransactionObject> => {
        const txn = (
            await fcl.send(
                await fcl.build([
        ...
        ) as FlowTransactionObject;
        return txn;
    }

    getTransactionStatus = async (transactionId: string): Promise<FlowTransactionStatus> => {
        const txn = await fcl.send(
            await fcl.build([
